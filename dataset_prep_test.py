...
siren_folder = os.path.join(BASE_DIR, "emergency_test")
traffic_folder = os.path.join(BASE_DIR, "urban_test")
output_base = os.path.join(BASE_DIR, "Dataset_test")
...
# Save fixed clips
def save_clips(src_folder, dst_folder, count=10):
...
# Mixed datasets
def save_mixed(dst_folder, first, second, count=10):
    folder_name = os.path.basename(dst_folder.rstrip("/"))
    for i in range(count):
